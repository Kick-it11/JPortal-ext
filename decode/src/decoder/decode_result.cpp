                                  ...
                                  PCStackInfo *pc, uint64_t entry)
{
    ...
    {
        if (entry == section->cmd()->osr_entry_point() && codelet_type == CodeletsEntry::_bytecode && (Bytecodes::is_branch(last_bytecode) || last_bytecode == Bytecodes::_goto || last_bytecode == Bytecodes::_goto_w))
            codelet_type = CodeletsEntry::_jitcode_osr_entry;
        else if (entry == section->cmd()->entry_point() || entry == section->cmd()->verified_entry_point())
