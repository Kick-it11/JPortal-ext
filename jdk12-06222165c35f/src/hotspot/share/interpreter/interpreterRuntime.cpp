...
IRT_ENTRY(address, InterpreterRuntime::exception_handler_for_exception(JavaThread* thread, oopDesc* exception))
  ...
  if (thread->frames_to_pop_failed_realloc() > 0) {
    ...
    return (address) -1;
#else
    return Interpreter::remove_activation_entry(h_method->is_jportal() && JPortalTrace);
#endif
  }
...
//     }
  ...
  if (handler_bci < 0 || !thread->reguard_stack((address) &continuation)) {
...
#ifndef CC_INTERP
    continuation = Interpreter::remove_activation_entry(h_method->is_jportal() && JPortalTrace);
#endif
    ...
    continuation = Interpreter::dispatch_table(vtos, h_method->is_jportal() && JPortalTrace)[*handler_pc];
#endif
  }
...
address SignatureHandlerLibrary::set_handler_blob() {
  BufferBlob* handler_blob = BufferBlob::create("native signature handlers", blob_size, false);
...
void SignatureHandlerLibrary::initialize() {
  ...
  BufferBlob* bb = BufferBlob::create("Signature Handler Temp Buffer",
                                      SignatureHandlerLibrary::buffer_size, false);
...
void SignatureHandlerLibrary::add(const methodHandle& method) {
  if (method->signature_handler() == NULL) {
    ...
    if (UseFastSignatureHandlers && method->size_of_parameters() <= Fingerprinter::max_size_of_parameters) {
      ...
      if (handler_index < 0) {
        ...
        InterpreterRuntime::SignatureHandlerGenerator(method, &buffer).generate(fingerprint, method->is_jportal() && JPortalTrace);
        // copy into code heap
        ...
        if (handler == NULL) {
          ...
          // debugging suppport
          if (PrintSignatureHandlers && (handler != Interpreter::slow_signature_handler(method->is_jportal() && JPortalTrace))) {
            ttyLocker ttyl;
            tty->cr();
...
#ifndef PRODUCT
            address rh_begin = Interpreter::result_handler(method()->result_type(), method->is_jportal() && JPortalTrace);
      ...
      if (handler_index < 0) {
        ...
        method->set_signature_handler(Interpreter::slow_signature_handler(method->is_jportal() && JPortalTrace));
      } else {
        // set handler
  ...
  assert(method->signature_handler() == Interpreter::slow_signature_handler(method->is_jportal() && JPortalTrace) ||
