...
void Parse::do_exits() {
  ...
  if (do_synch || C->env()->dtrace_method_probes() || _replaced_nodes_for_exceptions || (JPortalMethodNoinline && !_caller->has_method()) || JPortalMethodComp) {
    ...
    while ((ex_map = kit.pop_exception_state()) != NULL) {
      ...
      if (JPortalMethodNoinline && !_caller->has_method() && method()->is_jportal()
          || JPortalMethodComp && method()->is_jportal()) {
